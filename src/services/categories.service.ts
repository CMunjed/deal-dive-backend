...
export async function getCategoriesByName(names: string[]): Promise<Category[]> {
  if (!names.length) return [];

  const { data, error } = await supabase
    .from("categories")
    .select()
    .in("name_lower", names.map((n) => n.toLowerCase()));
...
export async function linkDealCategories(dealId: string, categoryNames: string[]): Promise<void> {
